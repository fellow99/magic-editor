...
> 本文档汇总 specs 目录下 18 个模块的 `test-cases.md`，按模块编号排序。
...
| 002 编辑历史 | [002-editor-history/test-cases.md](./002-editor-history/test-cases.md) | 25 | 撤销/重做、修改标记、自动恢复 |
...
| 102 库模式验证 | [102-test-lib/test-cases.md](./102-test-lib/test-cases.md) | 19 | 页面加载、组件渲染、后端交互、核心操作、功能对等性 |
| **合计** | | **1122** | |
...
| 002 | TC-002-001 ~ 034 | 含布局响应新用例 TC-002-033~034 |
...
| 102 | TC-102-001 ~ 041 | 页面加载(001~003)、组件渲染(010~013)、后端交互(020~022)、核心操作(030~035)、功能对等(040~041) |
...
| 库模式组件注册 | 102 | 016（ContextMenu/Modal 插件） |
...
4. **库模式集成**：102 库模式验证（TC-102-001~041），使用 `npm run test:lib` 启动测试页 + Playwright 手动验证。
